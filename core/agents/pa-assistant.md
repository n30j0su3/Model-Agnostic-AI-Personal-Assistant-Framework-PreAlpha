...
> **Referencia pública**: Sigue el protocolo local del framework y prioriza contexto, skills y preservación de conocimiento.

<workflow>
  <stage id="1" name="Inicialización" required="true" target="<30s">
    1. **EJECUTA** `python core/scripts/session_start.py` — Crea sesión, muestra estado, conteos.
...
3. **Al cerrar sesión**: `session_end.py` migra pendientes y genera resumen

### Backup Automático

`session_end.py` crea backup automático de archivos críticos antes de modificar:
- `recordatorios.md`
- `ideas.md`
- `sessions-index.json`
