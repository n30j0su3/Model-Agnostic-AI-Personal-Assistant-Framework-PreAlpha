...
def create_default_files():
    ...
    if not (KB_DIR / "README.md").exists():
        (KB_DIR / "README.md").write_text("""# Knowledge Base
...
**Ultima actualizacion**: """, encoding="utf-8")
    ...
    if not (prefs_dir / "preferences.md").exists():
        (prefs_dir / "preferences.md").write_text("""# Preferencias del Usuario
...
""", encoding="utf-8")
    ...
    if not (prefs_dir / "logging-config.md").exists():
        (prefs_dir / "logging-config.md").write_text("""# Configuracion de Logging
...
""", encoding="utf-8")
    ...
    if not (CODEBASE_DIR / "recordatorios.md").exists():
        (CODEBASE_DIR / "recordatorios.md").write_text("""# Pendientes
...
""", encoding="utf-8")
    ...
    if not (CODEBASE_DIR / "ideas.md").exists():
        (CODEBASE_DIR / "ideas.md").write_text("""# Ideas y Descubrimientos
...
def main():
    ...
    parser.add_argument(
        "--check", action="store_true", help="Solo verificar estado"
    )
    parser.add_argument(
