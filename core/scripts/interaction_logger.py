...
def print_stats(stats: Dict[str, Any], date: str):
    ...
    if stats.get('agents'):
        print("\nAgentes más usados:")
        for agent, count in sorted(stats.get('agents', {}).items(), key=lambda x: x[1], reverse=True)[:5]:
            print(f"  - {agent}: {count}")
    
    if stats.get('models'):
        print("\nModelos más usados:")
        for model, count in sorted(stats.get('models', {}).items(), key=lambda x: x[1], reverse=True)[:5]:
            print(f"  - {model}: {count}")
